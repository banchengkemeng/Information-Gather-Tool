...
class Output:
    ...
    def status_report(self, response, full_url):
        ...
        message = f"[+] [{status}]->{self.url+show_path}"
        m = f"[{status}]->{self.url+show_path}"
        if response.redirect:
            message = f"[+] [{status}]->{response.redirect}"
            m = f"[{status}]->{response.redirect}"
        for redirect in response.history:
            message = f"[+] [{status}]->{redirect}"
            m = f"[{status}]->{redirect}"
