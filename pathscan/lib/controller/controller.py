...
class Controller:
    ...
    def match_callback(self, path, response):
        ...
        m = self.output.status_report(response, self.options.full_url)[0]
        message = self.output.status_report(response, self.options.full_url)[1]
        if m not in self.ulist:
            self.output.new_line(message)
            self.ulist.append(m)
        # self.ulist.extend(self.output.status_report(response, self.options.full_url))
